...
namespace TaylorMadeCode.Core
{
    ...
    [System.Serializable]
    public class TMC_MonoBehaviour : MonoBehaviour, TMC_IObject, ISerializationCallbackReceiver
    {
        //--------------------- Required Variables ----------------------//
        ...
        {
            if (!mb_HasBeenSetup)
                Debug.LogError("A Taylor Made Code Script is detected as not being in a standard state, please reset the script to fix this issue", this);
        ...
        /// <summary>
        /// This function is just a function that call's RemoveScript(), This is done to have the correctly overriden version called by the context menu item.
        ...
        protected void ContextMenuTmcRemoveScript()
        {
            RemoveScript();
        }
        ...
        /// Basic function to allow specific setup code for each script. Not an abstract class as then you are forced to override for every script. However this isn't the best practice
        ...
        public void OnAfterDeserialize()
        {
            ...
            foreach (string StringToProcess in m_StringEffedDictionary)
            {
                string[] m_var = StringToProcess.Split("7C303332317C7C3036323232317C");
